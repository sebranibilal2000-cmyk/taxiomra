...
function Maintenance() {
  ...
  const columns: Column<any>[] = [
    { key: "service_date", header: "Date", render: (r) => new Date(r.service_date).toLocaleDateString() },
    ...
    { key: "mileage", header: "Mileage", render: (r) => r.mileage ? <span className="tabular-nums">{Number(r.mileage).toLocaleString()}</span> : "—" },
    { key: "cost", header: "Cost", render: (r) => <span className="tabular-nums font-medium">{Number(r.cost ?? 0).toFixed(2)}</span> },
    { key: "next_due_date", header: "Next due", render: (r) => <ExpiryPill date={r.next_due_date} /> },
  ];
  ...
  return (
    <div>
      <PageHeader title={locale === "ar" ? "الصيانة" : "Maintenance"} description={locale === "ar" ? "سجل الصيانة والتنبيهات" : "Service history and upcoming maintenance"} />

      <div className="grid gap-3 md:grid-cols-4 mb-6">
        <StatCard label="Records" value={records.data?.length ?? 0} icon={Wrench} />
        <StatCard label="Due soon" value={alerts} icon={AlertTriangle} tone="warning" />
        <StatCard label="This month" value={monthCost.toFixed(2)} icon={DollarSign} />
        <StatCard label="Total cost" value={totalCost.toFixed(2)} icon={DollarSign} tone="chart2" />
      </div>

      {alerts > 0 && (
        <Card className="mb-6 border-warning/40 bg-warning/5">
          <CardContent className="p-4">
            <div className="text-sm font-medium mb-3 flex items-center gap-2"><AlertTriangle className="h-4 w-4 text-warning-foreground" />Vehicles needing service</div>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        <div className="relative">
          ...
          <Input placeholder="Search plate, description, vendor…" value={search} onChange={(e) => setSearch(e.target.value)} className="ps-9 w-72" />
        </div>

...
function FleetDashboard() {
  ...
  return (
    <div>
      <PageHeader eyebrow="Overview" title={locale === "ar" ? "لوحة الأسطول" : "Fleet Dashboard"} description={locale === "ar" ? "حالة السائقين والمركبات في الوقت الحقيقي" : "Real-time driver and vehicle operations"} />

      <div className="grid gap-3 md:grid-cols-4 mb-6">
        <StatCard label="Vehicles" value={vList.length} icon={Car} />
        <StatCard label="Drivers" value={dList.length} icon={Users} />
        <StatCard label="Fleet utilization" value={`${utilization}%`} icon={TrendingUp} tone="chart2" />
        <StatCard label="Maint. cost (30d)" value={monthCost.toFixed(2)} icon={Wrench} tone="warning" />
      </div>

      <div className="grid gap-4 md:grid-cols-2 mb-6">
        <Card><CardContent className="p-5">
          <div className="text-[10px] uppercase tracking-widest text-muted-foreground mb-3">Vehicle status</div>
        ...
        <Card><CardContent className="p-5">
          <div className="flex items-center justify-between mb-3">
            <div className="text-[10px] uppercase tracking-widest text-muted-foreground">Driver status</div>
            <div className="text-[10px] text-muted-foreground">Utilization {driverUtil}%</div>
          </div>
          <div className="space-y-2">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card><CardContent className="p-5">
          <div className="flex items-center justify-between mb-3">
            <div className="text-[10px] uppercase tracking-widest text-muted-foreground flex items-center gap-2"><ShieldAlert className="h-3.5 w-3.5" />Vehicle alerts</div>
            <span className="text-xs text-muted-foreground">{alerting.length}</span>
          </div>
          {alerting.length === 0 ? <div className="text-sm text-muted-foreground py-6 flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-success" />All vehicles compliant.</div> :
              ...
              {alerting.slice(0, 20).map((v: any) => (
                <Link key={v.id} to={"/admin/fleet/$id" as any} params={{ id: v.id } as any} className="flex items-center justify-between py-2 hover:bg-muted/30 -mx-2 px-2">
                  ...
                  <div className="flex flex-wrap gap-1 justify-end max-w-[60%]">
                    {v.next_maintenance_date && daysUntil(v.next_maintenance_date)! <= 14 && <ExpiryPill date={v.next_maintenance_date} label="Service" />}
                    {v.registration_expiry && daysUntil(v.registration_expiry)! <= 30 && <ExpiryPill date={v.registration_expiry} label="Reg" />}
                    {v.insurance_expiry && daysUntil(v.insurance_expiry)! <= 30 && <ExpiryPill date={v.insurance_expiry} label="Ins" />}
                    {v.inspection_expiry && daysUntil(v.inspection_expiry)! <= 30 && <ExpiryPill date={v.inspection_expiry} label="Insp" />}
                  </div>
                </Link>
              ))}
            </div>
        ...
        <Card><CardContent className="p-5">
          <div className="flex items-center justify-between mb-3">
            <div className="text-[10px] uppercase tracking-widest text-muted-foreground flex items-center gap-2"><ShieldAlert className="h-3.5 w-3.5" />Driver alerts</div>
            <span className="text-xs text-muted-foreground">{driverAlerts.length}</span>
          </div>
          {driverAlerts.length === 0 ? <div className="text-sm text-muted-foreground py-6 flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-success" />All drivers compliant.</div> :
            <div className="divide-y max-h-80 overflow-y-auto">
              {driverAlerts.slice(0, 20).map((d: any) => (
                <Link key={d.id} to={"/admin/drivers/$id" as any} params={{ id: d.id } as any} className="flex items-center justify-between py-2 hover:bg-muted/30 -mx-2 px-2">
                  ...
                  <div className="flex flex-wrap gap-1 justify-end max-w-[65%]">
                    {d.license_expiry && daysUntil(d.license_expiry)! <= 30 && <ExpiryPill date={d.license_expiry} label="Lic" />}
                    {d.medical_expiry && daysUntil(d.medical_expiry)! <= 30 && <ExpiryPill date={d.medical_expiry} label="Med" />}
                    {d.work_permit_expiry && daysUntil(d.work_permit_expiry)! <= 30 && <ExpiryPill date={d.work_permit_expiry} label="Permit" />}
                    {d.insurance_expiry && daysUntil(d.insurance_expiry)! <= 30 && <ExpiryPill date={d.insurance_expiry} label="Ins" />}
                  </div>
                </Link>
              ))}
            </div>

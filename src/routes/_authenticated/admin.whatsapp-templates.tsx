...
function WhatsAppTemplatesAdmin() {
  ...
  const cols: Column<any>[] = [
    { key: "name", header: "Template", render: (r) => <div><div className="font-medium">{r.name}</div><code className="text-xs text-muted-foreground">{r.code}</code></div> },
    { key: "category", header: "Category", render: (r) => <span className="text-xs uppercase tracking-wider text-muted-foreground">{r.category}</span> },
    { key: "variables", header: "Variables", render: (r) => <span className="text-xs">{(r.variables ?? []).map((v: string) => `{{${v}}}`).join(" ")}</span> },
  ...
  return (
    <div>
      <div className="flex justify-between items-start mb-6">
        <PageHeader title="WhatsApp Templates" description="Reusable messages sent from bookings, invoices, and campaigns. Uses {{variables}} substituted at send time." />
          ...
          <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={(e) => { e.preventDefault(); void save(new FormData(e.currentTarget)); }} className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div><label className="text-sm">Code (unique)</label><Input name="code" defaultValue={editing?.code} required pattern="[a-z0-9_]+" /></div>
              ...
              <div><label className="text-sm">Body (EN)</label><Textarea name="body_en" defaultValue={editing?.body_en} rows={6} required /></div>
              <div><label className="text-sm">Body (AR)</label><Textarea name="body_ar" defaultValue={editing?.body_ar} rows={6} dir="rtl" required /></div>
              <div><label className="text-sm">Variables (comma-separated, e.g. name,code,pickup)</label><Input name="variables" defaultValue={editing?.variables?.join(", ") ?? ""} /></div>

...
function HeroAdmin() {
  ...
  const cols: Column<any>[] = [
    { key: "title_en", header: "Title", render: (r) => <div><div className="font-medium">{r.title_en}</div><div className="text-xs text-muted-foreground" dir="rtl">{r.title_ar}</div></div> },
    { key: "cta", header: "CTA", render: (r) => r.cta_href ? <span className="text-xs text-muted-foreground truncate max-w-[220px] inline-block">{r.cta_href}</span> : null },
    { key: "sort_order", header: "Order" },
    { key: "active", header: "Active", render: (r) => <Switch checked={r.active} onCheckedChange={async (v) => { await supabase.from("hero_slides").update({ active: v }).eq("id", r.id); qc.invalidateQueries({ queryKey: ["hero-admin"] }); }} /> },
  ...
  return (
    <div>
      <div className="flex justify-between items-start mb-6">
        <PageHeader title="Hero Slides" description="Rotating hero slides shown on the public homepage." />
        <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) setEditing(null); }}>
          <DialogTrigger asChild><Button><Plus className="h-4 w-4 me-2" />New Slide</Button></DialogTrigger>
          <DialogContent className="max-w-2xl">
            ...
            <form onSubmit={(e) => { e.preventDefault(); void save(new FormData(e.currentTarget)); }} className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><label className="text-sm">Subtitle (EN)</label><Textarea name="subtitle_en" defaultValue={editing?.subtitle_en} rows={2} /></div>
                <div><label className="text-sm">Subtitle (AR)</label><Textarea name="subtitle_ar" defaultValue={editing?.subtitle_ar} dir="rtl" rows={2} /></div>
              </div>

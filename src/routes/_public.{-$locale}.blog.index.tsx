...
function Blog() {
  ...
  return (
    <section className="container-tight py-16 md:py-24">
      ...
      {featured && (
        <Link to="/{-$locale}/blog/$slug" params={(prev) => ({ ...prev, slug: featured.slug })} className="group block mb-16">
      ...
      {rest.length > 0 && (
        <div className="border-t border-border/60 pt-12 grid gap-10 md:grid-cols-2 lg:grid-cols-3">
          {rest.map((p) => (
            <Link key={p.id} to="/{-$locale}/blog/$slug" params={(prev) => ({ ...prev, slug: p.slug })} className="group">
